...
export default function TerminalCore({
  ...
  const lastTabRef = useRef<{ value: string; time: number }>({ value: '', time: 0 })
  ...
  useEffect(() => {
    ...
    const unsubscribe = engine.subscribe((state) => {
      ...
      const lastOutput = [...state.lines].reverse().find((l) => l.type === 'output' || l.type === 'system' || l.type === 'error')
      if (lastOutput) {
  ...
  const handleMatrixComplete = useCallback(() => {
    ...
    if (engine) {
      ...
      const filtered = engine.getState().lines.filter((l) => l.type !== 'animation')
      engine.setLines([...filtered, { type: 'system', text: 'Exited the Matrix.' }])
    }
  }, [])
  ...
  return (
    <div className="terminal" onClick={handleOutputClick}>
      ...
      >
        {!booted && (
          <pre className="terminal-boot" aria-hidden="true">{bootText}<span className="terminal-cursor" /></pre>
        )}
        {booted && (
          <>
            <pre className="terminal-boot" aria-hidden="true">{isMobileViewport() ? BANNER_MOBILE : BANNER}</pre>
        ...
        <div className="sr-only" aria-live="assertive">{srAnnouncement}</div>
      </div>
      <StatusBar />
    </div>
  )
